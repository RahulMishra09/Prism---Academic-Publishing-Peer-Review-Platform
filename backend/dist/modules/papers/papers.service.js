 ...
 */
export const createPaper = async (authorId, input) => {
 ...
 * newest first.
 */
export const getMyPapers = async (authorId) => {
 ...
 */
export const listPapers = async (requesterId, requesterRole, query) => {
 ...
 */
export const getPaperById = async (paperId, requesterId, requesterRole) => {
 ...
 */
export const updatePaper = async (paperId, authorId, input) => {
    ...
    if (paper.status !== PaperStatus.DRAFT) {
        throw new AppError("Only papers in DRAFT status can be edited", 400);
    }
 ...
 */
export const submitPaper = async (paperId, authorId) => {
    const paper = await prisma.paper.findUnique({
        where: { id: paperId },
        select: { id: true, submittedBy: true, status: true },
    });
    ...
    if (paper.status !== PaperStatus.DRAFT) {
        throw new AppError(`Paper cannot be submitted from its current status (${paper.status})`, 400);
    }
    return prisma.paper.update({
        where: { id: paperId },
...
export const approvePaper = async (paperId) => {
    ...
    if (paper.status !== PaperStatus.SUBMITTED) {
        throw new AppError(`Only SUBMITTED papers can be approved (current: ${paper.status})`, 400);
    }
    ...
    if (completedReviews === 0) {
        throw new AppError("Paper must have at least one submitted review before it can be approved", 400);
    }
    return prisma.paper.update({
        where: { id: paperId },
        data: {
 ...
 */
export const rejectPaper = async (paperId, input) => {
    const paper = await prisma.paper.findUnique({
        where: { id: paperId },
        select: { id: true, status: true },
    });
    ...
    if (paper.status !== PaperStatus.SUBMITTED) {
        throw new AppError(`Only SUBMITTED papers can be rejected (current: ${paper.status})`, 400);
    }
