...
export const listAllPapers = async (query) => {
...
export const assignReviewer = async (paperId, input) => {
    ...
    if (paper.status !== PaperStatus.SUBMITTED) {
        throw new AppError(`Reviewers can only be assigned to SUBMITTED papers (current status: ${paper.status})`, 422);
    }
    ...
    if (reviewer.role !== Role.REVIEWER) {
        throw new AppError("The specified user does not have the REVIEWER role", 422);
    }
...
export const removeReviewer = async (paperId, reviewerId) => {
    ...
    if (assignment.status === "COMPLETED") {
        throw new AppError("Cannot remove a reviewer who has already submitted a review", 422);
    }
...
export const listAssignments = async (paperId) => {
    const paper = await prisma.paper.findUnique({
        where: { id: paperId },
        select: { id: true },
    });
