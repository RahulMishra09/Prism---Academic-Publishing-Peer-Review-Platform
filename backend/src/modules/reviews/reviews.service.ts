 ...
 */
export const getMyAssignments = async (reviewerId: string) => {
...
export const submitReview = async (
  ...
  if (assignment.paper.status !== PaperStatus.SUBMITTED) {
    throw new AppError(
      "Reviews can only be submitted for papers in SUBMITTED status",
      400
    );
  }
 ...
 */
export const getReviewsForPaper = async (
  paperId: string,
  requesterId: string,
  requesterRole: Role
) => {
 ...
 */
export const getMyReviews = async (reviewerId: string) => {
  return prisma.review.findMany({
    where:   { reviewerId },
    select:  reviewSelect,
    orderBy: { createdAt: "desc" },
