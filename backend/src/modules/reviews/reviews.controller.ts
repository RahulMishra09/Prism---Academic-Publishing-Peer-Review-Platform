...
export const submit = async (
  ...
  try {
    ...
    if (!parsed.success) {
      ...
      const exists = await import("../../config/prisma.js").then(m =>
        m.prisma.reviewerAssignment.findUnique({
 ...
 */
export const listForPaper = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    const reviews = await getReviewsForPaper(
