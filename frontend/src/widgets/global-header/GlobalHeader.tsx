...
import { FaBars, FaTimes, FaSearch, FaUser, FaSignOutAlt, FaSun, FaMoon, FaBell } from 'react-icons/fa';
import { MegaMenu } from '../mega-menu/MegaMenu';
import { useAuthStore } from '../../app/store/useAuthStore';
import { useThemeStore } from '../../entities/theme/model/useThemeStore';
import { canViewEditorDashboard, canReview } from '../../shared/lib/roles';
import { useAlertsQuery } from '../../features/user/api/useUserDashboard';
...
export function GlobalHeader() {
    ...
    const { data: alerts } = useAlertsQuery(isAuthenticated);
    const alertCount = isAuthenticated ? (alerts?.length ?? 0) : 0;
                    ...
                    >
                        {isAuthenticated && user ? (
                            <div className="flex items-center gap-5">
                                ...
                                {canViewEditorDashboard(user.role) && (
                                    <Link
                                        to="/editor"
                                        className="text-lumex-muted hover:text-lumex-blue transition-colors"
                                    >
                                ...
                                <Link
                                    to="/account"
                                    className="relative flex items-center text-lumex-muted hover:text-lumex-blue transition-colors"
                                    onClick={() => { /* navigate to alerts tab */ }}
                                >
                                    <FaBell size={12} />
                                    {alertCount > 0 && (
