...
export const CheckoutPage: React.FC = () => {
    ...
    if (isLoading) {
        return (
            <Container className="py-20 text-center">
                <p className="text-lumex-muted">Loading checkout...</p>
            </Container>
        );
    }
    ...
    if (!article) {
        return (
            <Container className="py-20 text-center">
                <h1 className="text-2xl font-bold text-lumex-text">Article not found</h1>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                <div className="lg:col-span-2">
                    ...
                    <section className="bg-white border border-lumex-border rounded-lg p-6 mb-8">
                        ...
                        <div className="mb-4">
                            <h3 className="font-bold text-lumex-text mb-1 leading-snug">{article.title}</h3>
                            <p className="text-sm text-lumex-muted mb-1">{article.authors.map((a: Author) => `${a.firstName} ${a.lastName}`).join(', ')}</p>
                            <p className="text-xs text-lumex-muted italic">{article.journalTitle}</p>
                        </div>
                                ...
                                <div className="flex justify-between items-start">
                                    <div className="flex gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-lumex-muted mt-1">Read the full text online for 48 hours.</p>
                                        </div>
                                    </div>
                                ...
                                <div className="flex justify-between items-start">
                                    <div className="flex gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-lumex-muted mt-1">Read online and download PDF indefinitely.</p>
                                        </div>
                                    </div>
                ...
                <div className="lg:col-span-1">
                    {/* Payment Info */}
                    <section className="bg-lumex-bg-deep/40 border border-lumex-border rounded-lg p-6 sticky top-20">
                        <h2 className="text-xl font-bold text-lumex-text mb-6 border-b border-lumex-border pb-2">
                            Payment Details
                        </h2>

                        <form onSubmit={handleCheckout} className="space-y-5">
                            <Stack>
                                <label className="text-sm font-bold text-lumex-text">Cardholder Name</label>
                                <Input placeholder="Name on card" required />
                            </Stack>
                            <Stack>
                                <label className="text-sm font-bold text-lumex-text">Card Number</label>
                                <Input placeholder="0000 0000 0000 0000" required pattern="\d*" maxLength={16} />
                            </Stack>
                            <div className="flex gap-4">
                                <Stack className="flex-1">
                                    <label className="text-sm font-bold text-lumex-text">Expiry</label>
                                    <Input placeholder="MM/YY" required maxLength={5} />
                                </Stack>
                                <Stack className="flex-1">
                                    <label className="text-sm font-bold text-lumex-text">CVC</label>
                            ...
                            <div className="pt-6 border-t border-lumex-border">
                                <div className="flex justify-between items-center mb-6">
                                    <span className="font-bold text-lumex-text">Total Price</span>
                                ...
                                <p className="text-xs text-center text-lumex-muted mt-4 leading-relaxed">
